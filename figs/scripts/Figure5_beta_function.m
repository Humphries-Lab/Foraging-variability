...
model = [3 6 7 9 10 12 13]; % compare MVT and Block models
modelNames = {'vary \beta vary c','scalar MVT', 'scalar \rho',...
              'exp MVT', 'exp \rho',...
              'hyp MVT', 'hyp \rho'};
...
for s = 1:numel(study)
    ...
    ylabel('BIC (sum)')
    set(gca, 'XLim', [0,nModels+1], 'XTickLabel',modelNames, 'XTickLabelRotation',90, 'YLim', [min(models_BIC)-200,max(models_BIC)+200])

    FormatFig_For_Export(gcf,fontsize,fontname,widths.axis)
    if save_figs == 1
        print([export_path, sprintf('fig5_%s_BIC', study{s})],'-dsvg')
        print([overleaf_path, 'fig5/', sprintf('fig5_%s_BIC', study{s})],'-dsvg')
    end

    % compute and plot posterior probabilities for each subject
    posteriorProbabilities = BICposterior(squeeze(ppts_BIC(:,:,[bestModel,1]))); % difference between winning model, and the winning fit beta model from figure 3

    figure('Units', 'centimeters', 'PaperPositionMode', 'auto' ,'Position',figsize.rectangle);

    posteriors = posteriorProbabilities(:,1) - posteriorProbabilities(:,2);
    ...
    FormatFig_For_Export(gcf,fontsize,fontname,widths.axis)
    if save_figs == 1
        print([export_path, sprintf('fig5_%s_subject_posteriors', study{s})],'-dsvg')
        print([overleaf_path, 'fig5/', sprintf('fig5_%s_subject_posteriors', study{s})],'-dsvg')
    end
    clear ppts_BIC models_BIC posteriorProbabilities
end

%% Panels: simulated vs empirical mean leaving times
% for winning model, M7
for s = 1:numel(study)
    task = buildTask(study{s});

    load(sprintf('fitting_results_M7_%s',study{s}), '-mat');
    load(sprintf('subjectLT_%s', study{s}), '-mat');
    load(sprintf('modelLT_%s_M7', study{s}), '-mat')
    ...
    FormatFig_For_Export(gcf,fontsize,fontname,widths.axis)
    if save_figs == 1
        print([export_path, sprintf('fig5_%s_simulated_mean_LT', study{s})],'-dsvg')
        print([overleaf_path, 'fig5/', sprintf('fig5_%s_simulated_mean_LT', study{s})],'-dsvg')
    end
    ...
    if save_figs == 1
        ...
        print([overleaf_path, 'fig5/', sprintf('fig5_%s_simulated_mean_LT_overlap', study{s})],'-dsvg')
    end
...
for s = 1:numel(study)
    ...
    load(sprintf('modelLT_all_%s_M7', study{s}), '-mat')
    ...
    if save_figs == 1
        ...
        print([overleaf_path, 'fig5/', sprintf('fig5_%s_per_participant', study{s})],'-dsvg')
    end
...
for s = 1:numel(study)
    ...
    load(sprintf('fitting_results_M7_%s',study{s}), '-mat', 'minNLLFitParams');

    load(sprintf('model_betas_%s_M7',study{s}), '-mat');

    mean_betas = squeeze(mean(betas,1))'; % betas are constant in each environment anyway, this only matters for when doing RW model

    figure('Units', 'centimeters', 'PaperPositionMode', 'auto' ,'Position',figsize.square); hold on
    xlabel('Subject leaving times (s)');
    ylabel('Estimated beta fit');

    scatter(subjMean, minNLLFitParams.lambda, 50, "MarkerFaceColor",color.general, "MarkerEdgeColor",'w','MarkerFaceAlpha', 0.5,'MarkerEdgeAlpha',0.5)
            figure, scatter(subjMean, minNLLFitParams.bias, 50, "MarkerFaceColor",color.general, "MarkerEdgeColor",'w','MarkerFaceAlpha', 0.5,'MarkerEdgeAlpha',0.5)

    FormatFig_For_Export(gcf,fontsize,fontname,widths.axis)
    if save_figs == 1
        print([export_path, sprintf('fig5_%s_lambda_LT_supplement', study{s})],'-dsvg')
        print([overleaf_path, 'fig5/', sprintf('fig5_%s_lambda_LT_supplement', study{s})],'-dsvg')
    end

        [r p] = corr(subjMean',minNLLFitParams.lambda, 'type', 'Pearson')
        [r p] = corr(subjMean',minNLLFitParams.bias, 'type', 'Pearson')
