function [NegLogLikelihood, out] = NLL_MVT_softmax_dynamicbeta_threeParams(params, Env, SubjData)
%% Set up
...
for ii = 1:BlockTime-1 % for each subject action
    ...
    if Action(ii) == Stay % take action to stay
        ...
        Beta = Kappa + Lambda * 1/(experiencedAvgRR(ii)^Omega); % beta function based on avgRR
