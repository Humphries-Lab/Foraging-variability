function [NegLogLikelihood, out] = NLL_M1_MVT(params, Env, SubjData, block)
%% Set up
...
PAction = zeros(BlockTime,2); % the reward earned in each state in the block
...
%Rho = ones(BlockTime+1, 1)*Env.OptimalAverageRR{block}; % initialise to actual averageRR
...
for ii = 1:BlockTime-1 % for each second in the environment
    ...
    if Action(ii) == Stay % take action to stay
        ...
        PAction(ii + 1,:) = EstimatedPatchRR(ii+1) > Rho(ii+1);
