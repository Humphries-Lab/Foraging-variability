function [output, Q_stay, Q_leave, rho, numObservations] = simulate_RL_off_policy(params, environ, block, model_table, model)
...
patchRR = zeros(environ.block_time+1,1); % reward prediction error
...
switch patch_sequence
    ...
    case 'single'
        Q_stay = zeros(environ.block_time+1, 1);
end
...
for ii = 1:environ.block_time
    ...
    if Actions(ii) == stay % take action to stay
        ...
        RPE(ii) = (patch_reward(ii)/57.5 - rho(ii)) + max(Q(ii+1, :)) - Q(ii, Actions(ii));
        ...
        if Actions(ii+1) == leave
            ...
            T_at_leaving = T; % log the state when they left, for off policy Q indexing
