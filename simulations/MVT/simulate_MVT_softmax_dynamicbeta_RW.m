function [out] = simulate_MVT_softmax_dynamicbeta_RW(params, Env, startValues)
%% Set up

AlphaRho = params(1);
Lambda = params(2); % softmax temperature
...
Rho = zeros(Env.BlockTime/Env.TimeStep+1,1); % the reward rate in each state in the block
Rho(1) = Env.OptimalAverageRR{Env.CurrentBlock};
...
for ii = 1:Env.BlockTime/Env.TimeStep % for each second in the environment
    ...
    if Action(ii) == Stay % take action to stay
        ...
        Beta(ii) = Lambda * 1/Rho(ii+1); % beta function based on avgRR
...
out.Rho = Rho(1:Env.BlockTime/Env.TimeStep);
