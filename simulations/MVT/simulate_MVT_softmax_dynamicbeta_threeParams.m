function [out] = simulate_MVT_softmax_dynamicbeta_threeParams(params, Env, startValues)
%% Set up
...
for ii = 1:Env.BlockTime/Env.TimeStep % for each second in the environment
    ...
    if Action(ii) == Stay % take action to stay
        ...
        Beta(ii) = Kappa + Lambda * 1/(experiencedAvgRR(ii)^Omega); % beta function based on avgRR
