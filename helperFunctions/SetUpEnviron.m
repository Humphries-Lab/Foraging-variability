...
for j=1:length(Env.InitialYield) % for each yield type
    ...
    Env.RR(:,j) = fun(0:Env.BlockTime); % for theoretical maximum of Block Time in the patch
    k = 0:Env.TimeStep:Env.BlockTime; %this is set for a maximum of block_time secs in the patch, with loop time according to time_step
    for i = 1:length(k)
...
% patch order in rich environment - taken from Le Heron scripts 
...
% patch order in poor environment - taken from Le Heron scripts 
