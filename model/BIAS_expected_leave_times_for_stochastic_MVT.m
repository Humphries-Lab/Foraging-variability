...
% 
% 11 Jan 2024 Emma Scholey: adapt script to sweep bias term instead of beta
% 
...
for iR = 1:numel(task.r0)
    ...
    for iB = 1:numel(bias_parameter)
        p_leave_at_n = p_leave_softmax(reward_ts,explore_parameter, bias_parameter(iB),0);
...
% find corresponding values of bias for expected leaving time of mid patch 
ixRich = find(E_leave(:,2) >= mean_leave_data(1,2),1,"last");
ixPoor = find(E_leave(:,2) >= mean_leave_data(2,2),1,"last");

bias_rich = bias_parameter(ixRich);
bias_poor = bias_parameter(ixPoor);

%% plot predictions of means and SDs for these bias values 

% draw as coloured lines on above E and SD figs
figure(E_fig)
line([bias_rich bias_rich],[0 E_fig.Children.YLim(2)],'Color',color.rich)
line([bias_poor bias_poor],[0 E_fig.Children.YLim(2)],'Color',color.poor)
...
%% plot expected leaving time for these bias values, compared to experimental data
figure
...
ylabel('Expected leaving time (s)')
title(sprintf('bias rich = %f, bias poor = %f', round(bias_rich,3),round(bias_poor,3)))
...
% 
% %% reward rate predictions - plot reward rates for range of bias parameter
% for iR = 1:numel(task.r0)
%     switch task.rewardFunction
%         case 'exponential'
...
% 
% RR_fig = figure;
% semilogx(bias_parameter,RR_leave); hold on
% switch model
%     case {'softmax','lapse'}
%         xlabel('Bias (higher = exploit)')
%     case 'e-greedy'
%         xlabel('\epsilon (higher = bias)')
%         %set(gca,'XDir','reverse')
% end
% ylabel('Reward rate at expected leaving time (s)')
% 
% line([bias_rich bias_rich],[0 RR_fig.Children.YLim(2)],'Color',color.rich)
% line([bias_poor bias_poor],[0 RR_fig.Children.YLim(2)],'Color',color.poor)
