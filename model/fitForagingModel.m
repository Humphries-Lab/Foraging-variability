...
% load task
task = buildTask(fitOptions.study,fitOptions.blockPresentation);
...
for modelNum = 27 % for all models
    ...
    minNLL = zeros([nSub, task.numFitBlocks]);
    minNLLFitParams = zeros([nSub, allParams.nParams, task.numFitBlocks]);
    BIC = zeros([nSub, task.numFitBlocks]);
    AIC = zeros([nSub, task.numFitBlocks]);
    ...
    for iS = 1:nSub
        ...
        for iB = 1:task.numFitBlocks 
            ...
            subj.switchIndex = allData.switchIndex{iS,iB};
...
%     end
    ...
    if strcmp(fitOptions.blockPresentation, 'separate')
        ...
        minNLLFitParams_poor = array2table(minNLLFitParams(:,:,2), "VariableNames",model.paramNames);
         save_name = sprintf('../data/fitting_data_240104/fitting_results_separate_M%d_%s', model.modelNumber,fitOptions.study);
    ...
    elseif strcmp(fitOptions.blockPresentation, 'combined')
        ...
        save_name = sprintf('../data/fitting_data_240104/fitting_results_combined_M%d_%s', model.modelNumber,fitOptions.study);
