function [NegLogLikelihood,rho,Q_stay,Q_leave,PAction,numObservations] = NLL_subject_foraging_RL_on(params,Data,patch_order,environ,model_table,model)
...
for ii = 1:time_in_block-1
    ...
    if Actions(ii) == stay % take action to stay
        ...
        RPE(ii) = (Rewards(ii)/57.5 - rho(ii)) + Q(ii+1, Actions(ii+1)) - Q(ii, Actions(ii));
