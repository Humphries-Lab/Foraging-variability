function [minNLL, minNLLFitParams, BIC, AIC, FitParams, NLLEval, StartParams] = fitM1_MVT_RWRho(SubjData, Env, nStarts, maxAlpha)
...
%% Run fitting
f = @(x)NLL_M1_MVT_RWRho(x, Env, SubjData);

parfor ii = 1:nStarts
    params0 = [0.001+rand(1,1)*(maxAlpha(2)-0.001), exprnd(10)]; % choose start parameters
...
%% Calculate BIC/AIC
[~, out] = NLL_M1_MVT_RWRho([0 1], Env, SubjData); % get the number of data points that were fit to (parameter values don't matter here)
